...
def read():
    ...
    # Pressure
    	press_data = data[0]+np.float(data[1])

    # Temperature
	temp_data = (data[3]+np.float(data[4]))

    # Calibration check
	cali_data = (data[6]+np.float(data[7]))
...
def pressure(press_data):
	press_data = press_data/255
	if press_data >= 0 and press_data<128:
		diff_press = press_data*60/256
	elif press_data>128 and press_data<=256:
		diff_press=-(256-press_data)*60/256
	return diff_press
        
# Conversion counts to degree Celcius	
def temperature(temp_data):
	return np.float(temp_data)/10.0
